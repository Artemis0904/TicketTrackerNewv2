...
-- Recreate trigger to avoid IF NOT EXISTS issues
DROP TRIGGER IF EXISTS update_profiles_updated_at ON public.profiles;
CREATE TRIGGER update_profiles_updated_at
BEFORE UPDATE ON public.profiles
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- RLS policies for profiles (drop then create to be idempotent)
...
-- New user trigger to populate profiles
...
CREATE POLICY "Anyone authenticated can view tickets"
...
DROP POLICY IF EXISTS "Regional managers can delete any ticket" ON public.tickets;
CREATE POLICY "Regional managers can delete any ticket"
